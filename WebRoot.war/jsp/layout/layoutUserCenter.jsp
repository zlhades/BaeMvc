            ...
            <script type="text/javascript">
                ...
                $(function(){
                    ...
                    $("#tip_msg_inner").hover(function(){
                        tipInterval = clearInterval(tipInterval);
                    },function(){
                    ...
                    $("#close_btn").click(function(){
                        $("#pop_live_message_tip").slideUp("slow");
                        cycleInterval = clearInterval(cycleInterval);
                    });

                    function showLiveTipBox(){
                        //这里的代码判断是否有粉丝开始了直播--AJAX
                        if('<s:property value="#session.userID"/>' != ""){
                              //  var existLive = true;
                                $.post("findUserFanPerOnlive.html",function(data){
                                //    alert(data.userName != null && data.personalOnliveID != null && !data.perOnliveFinished);
                                     if(data.userName != null && data.personalOnliveID != null && !data.perOnliveFinished){
                                          $("#userName_perHost").html(data.userName);
                                          $("#user_pic").attr("src",data.userLogoUrl);
                                          $("#linkAddress_perHost").attr('href','fansWatchPerOnliveControl.html?personalOnlive.id='+data.personalOnliveID+'&userMatters.id='+data.userMattersID);
                                ...
                                cycleInterval = setTimeout(function(){showLiveTipBox();},180000);
                        }
                    }
                });
            </script>
